...
const Products = (props) => {
    ...
    return (
        <div className={"row card-container m-2"}>
            ...
            <div className={"d-flex justify-content-between fixed-custom pe-4 mb-2"}>
                <div className={"m-1 ms-5 ps-3"}>
                    <img className={"filter-img"} src={filters} alt={"filters"}
                         onClick={() => handleToggleFilters()}/>
                </div>
                <select className="form-select sort-dropdown me-5" value={sortOrder} onChange={handleSortChange}>
            ...
            <div className={"col-2 sidebar p-3 mb-3 mt-5 d-none"}>
                ...
                <div className={"text-center m-2 p-0 filterWrap"}>
                    <a className={"filter text-center link"} onClick={clearAllFilters}>
                        <h5 className={"py-2"}>Clear filters</h5>
                    </a>
                </div>
            </div>
            <div className="col">
                <div className={"row d-flex m-5 mt-3"}>
                    {props.products.length === 0 ?
                        <h2 className={"col px-4 text-center"}>No products matched your search</h2> :
                        props.products.map((p) => {
                            return (
                                <Link className="card-wrapper col-3 mb-3" onClick={() => props.onDetails(p.id)} to={`/product/${p.id}`} key={p.id}>
                                    <div className="card product-card rounded-0 border-0 mx-1 d-flex flex-column justify-content-between">
                                        <div>
                                        ...
                                        <div>
                                            <div className="card-body">
                                                ...
                                                {roles == "ROLE_USER" &&
                                                    <Link onClick={() => handleAddToCart(p.id)} className="btn btn-info rounded-0 w-100 p-2 fs-5"><span className={"fa fa-shopping-cart"}></span> Add To Cart</Link>
                                                }
                                                {roles == "ROLE_ADMIN" &&
                                                    <>
